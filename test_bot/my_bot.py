...
from .q_learning import determine_state, choose_action, learn_actions, QAction, QState, MistDistance, MistDirection, \
    update_q_values, get_table, q_values
...
class BotController:
    ...
    def init_enviroment_map(self, map_name: str):
        ...
        try:
            ...
            arena = [[self.terrain_mapping(terrain, coordinates.Coords(*(x, y))) for x in range(size[0])] for y in
                     range(size[1])]
            self.arena = arena
    ...
    def check_enemies_in_neighbourhood(self):
        ...
        target_position = self.enemies_knowledge[closest_alive_enemy][
            1] if closest_alive_enemy is not None else None  # coord
    ...
    def get_enemies(self, knowledge):
        enemies = dict(filter(lambda elem: elem[1].character != None and elem[0] != self.bot_position,
                              knowledge.visible_tiles.items()))
    ...
    def update_weapons_knowledge(self, knowledge: characters.ChampionKnowledge) -> None:
        ...
        self.weapons_knowledge = dict(filter(
            lambda elem: WEAPONS_DESCRIPTORS[elem[1]] > WEAPONS_DESCRIPTORS[self.weapon] and elem[
                0] != self.bot_position, self.weapons_knowledge.items()))

        new_dynamic_obstacles = copy.deepcopy(self.weapons_knowledge)
        new_dynamic_obstacles = dict(filter(
            lambda elem: WEAPONS_DESCRIPTORS[elem[1]] <= WEAPONS_DESCRIPTORS[self.weapon] and elem[
                0] != self.bot_position, new_dynamic_obstacles.items()))
        self.dynamic_obstacles = {**self.dynamic_obstacles, **new_dynamic_obstacles}

    def determine_next_weapon(self):
        temp_weapons_knowledge = dict(filter(
            lambda elem: WEAPONS_DESCRIPTORS[elem[1]] > WEAPONS_DESCRIPTORS[self.weapon] and elem[
                1] != self.bot_position, self.weapons_knowledge.items()))
        weapons_scores = {k: 2 * WEAPONS_DESCRIPTORS[v] / (1 + g_distance(self.bot_position, k)) for k, v in
                          temp_weapons_knowledge.items()}
        if weapons_scores:
    ...
    def find_vector_to_nearest_mist_tile(self, knowledge: characters.ChampionKnowledge):
        ...
        for coord in visible_tiles.keys():
            if g_distance(self.bot_position, coord) <= SIGHT_RANGE and self.is_given_type_tile(knowledge, coord,
                                                                                               MIST_DESCRIPTOR):
    ...
    def choose_best_direction(self):
        ...
        possible_coordinates = [pos for pos in possible_coordinates if
                                (self.arena_size[0] > pos[0] >= 0) and (self.arena_size[1] > pos[1] >= 0)]
